...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus,
        ...
        # 2048 -> 1024
        self.encoder = self.transfer(tf.add(tf.matmul(self.x + scale * tf.random_normal((n_input,)),
                                                     self.weights['w1']), self.weights['b1']))
        # 1024 -> n_hidden
        self.hidden = self.transfer(tf.add(tf.matmul(self.encoder, self.weights['w2']), self.weights['b2']))
        # n_hidden -> 1024
        self.decoder = self.transfer(tf.add(tf.matmul(self.hidden, self.weights['w3']), self.weights['b3']))
        # 1024 -> 2048
        self.reconstruction = self.transfertf.add(tf.matmul(self.decoder, self.weights['w4']), 
                                               self.weights['b4']) # 不需要使用激活函数
    ...
    def _initialize_weights(self):
        # 参数初始化函数
        all_weights = dict()
        all_weights['w1'] = tf.Variable(xavier_init(self.n_input, 1024))
        all_weights['b1'] = tf.Variable(tf.zeros([1024], dtype = tf.float32))
        all_weights['w2'] = tf.Variable(xavier_init(1024, self.n_hidden))
        all_weights['b2'] = tf.Variable(tf.zeros([self.n_hidden], dtype = tf.float32))
        all_weights['w3'] = tf.Variable(tf.zeros([self.n_hidden, 1024], dtype = tf.float32))
        all_weights['b3'] = tf.Variable(tf.zeros([1024], dtype = tf.float32))
        all_weights['w4'] = tf.Variable(tf.zeros([1024, 2048], dtype = tf.float32))
        all_weights['b4'] = tf.Variable(tf.zeros([2048], dtype = tf.float32))
...
new_train_data = autoencoder.transform(X_train, _scale=0)
np.savetxt(trainDat_name, new_train_data)
print('train data save successfully.')

new_test_data = autoencoder.transform(X_test, _scale=0)
