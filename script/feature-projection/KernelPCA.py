...
for DimRedu in dim_range:
	pca = KernelPCA(n_components=DimRedu, kernel=kernel)
	...
	trainX_name = './data_process(float)/kernel_pca/(' + kernel + ')pca_dim_' + str(DimRedu) + '_train_data.txt'
	testX_name = './data_process(float)/kernel_pca/(' + kernel + ')pca_dim_' + str(DimRedu) + '_test_data.txt'
	trainy_name = './data_process(float)/kernel_pca/(' + kernel + ')pca_dim_' + str(DimRedu) + '_train_label.txt'
	testy_name = './data_process(float)/kernel_pca/(' + kernel + ')pca_dim_' + str(DimRedu) + '_test_label.txt'
	...
	print('kernel_pca(dim=' + str(DimRedu) + ') finished.\n')
