...
for i in [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05]:
    lsvc = LinearSVC(C = i, penalty="l1", dual=False).fit(data, label)
    ...
    np.savetxt("./Data/data_reduction/data_train"+ "_fs3_"+str(dim)+"_.txt", train_data)
    # np.savetxt("./Data/data_reduction/label_train.txt", train_label)
    np.savetxt("./Data/data_reduction/data_test"+"_fs3_"+str(dim)+"_.txt", test_data)
